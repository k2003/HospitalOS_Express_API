...
export class serviceModel {
  ...
  detail(knex: Knex, id: string, limit: number = 50 , offset: number = 0) {
    return knex(this.tableName)
    .column('t_visit.t_visit_id','t_visit.visit_vn','t_visit.visit_dx','t_visit.f_visit_type_id'
          ...
          't_visit_primary_symptom.visit_primary_symptom_current_illness as cc')
            .column(knex.raw('string_agg(distinct (d.diag_icd10_number), \' : \') as diag_icd10_number '))
   ...
   // .where('d.f_diag_icd10_type_id','1')
    .whereNot('t_visit_primary_symptom.visit_primary_symptom_main_symptom','')
      ...
      .groupBy('t_visit.t_visit_id','fname','lname','mainsymp','cc')
      .limit(limit)
      .offset(offset);
  }
